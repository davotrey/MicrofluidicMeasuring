...
def findThreshDry(filename, showDry):
    ...
    cv2.rectangle(frame,(650,410),(690,500),(0,255,0),1)    # Draw a rectangle.
    dryDevicePixels = []                                    # initialize region
    for i in range(750,790):                                   # cycle through the rows
...
def FindRGBValues(filename):
    ...
    cv2.line(image, (200,399), (1000,399), (178, 34, 34), 1)                   # Draw a line to show the scanning region.
    ...
    # Red graph was 16 pixels wide.

    # red = np.zeros(1280)                    # Initialize an numpy array with 100 elements, filling it with zeros
    # blue = np.zeros(1280)
    # green = np.zeros(1280)
    # gray = np.zeros(1280)
    # for i in range(0,1270):                 # For 100 increments, store data points fromf the numpy arrays to the important portion.
    #     red[i] = dataRGB[i,RED]               # Red is in column 0 of the tuple, start adding elements at row 650 just before the color change from background to device.
    #     blue[i] = dataRGB[i,BLUE]              # Blue is in column 2 of the tuple.
    #     green[i] = dataRGB[i,GREEN]             # Green is in column 1 of the tuple.
    #     gray[i] = dataGray[i]               # Gray just needs the row to be selected properly, it is not a matrix like the dataRGB numpy array is.

    red = np.zeros(LENGTH_OF_SCAN)                     # Initialize an numpy array with 100 elements, filling it with zeros
    blue = np.zeros(LENGTH_OF_SCAN)
    green = np.zeros(LENGTH_OF_SCAN)
    ...
    for i in range(0,LENGTH_OF_SCAN):                  # For 100 increments, store data points from the numpy arrays to the important portion.
        red[i] = dataRGB[i+START_OF_SCAN,RED]           # Red is in column 0 of the tuple, start adding elements at row 650 just before the color change from background to device.
        blue[i] = dataRGB[i+START_OF_SCAN,BLUE]          # Blue is in column 2 of the tuple.
        green[i] = dataRGB[i+START_OF_SCAN,GREEN]         # Green is in column 1 of the tuple.
    ...
    for x in range(60,140):
        print("column:",x,"Red Value:",gray[x])
    backgroundAverage= 0
    deviceAverage = 0
    backgroundSum = 0
    deviceSum = 0
    MEAN = 10                               # Take an average over 50 points.
    for x in range(81-MEAN,81):
        backgroundSum = backgroundSum + gray[x]
    backgroundAverage = backgroundSum / MEAN
    for x in range(129,129+MEAN):
        deviceSum = deviceSum + gray[x]
    deviceAverage = deviceSum / MEAN
    print("gray background average: ", backgroundAverage, "gray device average: ", deviceAverage)
    leftBound = 0
    rightBound = 0
    for x in range(100,120):
        if (gray[x] < backgroundAverage):
            print ("LEFT UNCERTAINTY BOUND column: ",x,"gray: ",gray[x])
            leftBound = x
            break
    for x in range(120,150):
        if (gray[x] < deviceAverage):
            print ("RIGHT UNCERTAINTY BOUND column: ",x,"gray: ",gray[x])
            rightBound = x
            break
    rangeOfUncertainty = rightBound - leftBound
    print("Range of Uncertainty: ",rangeOfUncertainty)
    # plt.plot(red, 'ro')                     # Plot the red values with red dots.
    # plt.plot(blue, 'bo')                    # Plot the blue values with blue dots.
    # plt.plot(green, 'go')                   # Plot the green values with green dots.
...
# filmDepth("F_6.5_WaterFiber.jpg","F_6.5_Dry.jpg","F_6.5_Wet.jpg",1,1,1) # Call the function with three files and three ints that tell the function whether to print images and debugging
FindRGBValues("F_6.5_Wet.jpg")
