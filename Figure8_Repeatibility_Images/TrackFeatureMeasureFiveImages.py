...
def findWidestPoint(frame):
    column_scan = []
    row = [] 
    for i in range(TOP,BOTTOM):                     # Iterate between the top and bottom of the screen.
        for j in range(LEFT_SIDE,RIGHT_SIDE):         # Iterate between halfway on the screen to the right quarter of the screen, which should include the device.
    ...
    columnValue = RIGHT
    edgeOfDeviceIndex = 0
    for i in range(len(column_scan)):
        if column_scan[i] <= columnValue:
            columnValue = column_scan[i]
            edgeOfDeviceIndex = i
    return edgeOfDeviceIndex
...
def filmDepth(fiberFilename, dryFilename, wetFilename, showFiber, showDry, showWet,leftX1,rightX1,topY1,botY1,leftX2,rightX2,topY2,botY2):
    ...
    dryThreshVal = findThreshValue(dryFilename,leftX1,rightX1,topY1,botY1,0,leftX2,rightX2,topY2,botY2)   # Find the thresholding value by calling findThreshDry function and setting it to dryThresh.
    print ("dryThresh: ",dryThreshVal)
    ...
    wetThreshVal = findThreshValue(wetFilename,leftX1,rightX1,topY1,botY1,0,leftX2,rightX2,topY2,botY2)   # Calls the findThreshWet function to find the thresholding value.
    print ("wetThresh: ",wetThreshVal)
    ...
    landmarkForScan = findWidestPoint(dryThresh)
    global startOfScan 
    startOfScan = landmarkForScan +300
    # startOfScan = 450
    endOfScan = startOfScan + 9
    print("WidestPoint: ", landmarkForScan, "Start of scan: ",startOfScan)

    cv2.line(wetFrame, (LEFT_SIDE,startOfScan), (RIGHT_SIDE,startOfScan), (255, 0, 0), 1)
    cv2.line(wetFrame, (LEFT_SIDE,landmarkForScan), (RIGHT_SIDE,landmarkForScan), (0, 0, 255), 1)
    if showWet == SHOW:
        cv2.imshow('Wet Device Original', wetFrame)     # Show the original device.
        cv2.imshow('Wet Device Thresholded', wetThresh) # Show the thresholded device.
    cv2.waitKey(0)                                  # Wait for a key press from the user.  

    for i in range(startOfScan,endOfScan):                            # Scan each row on the device.
...
filmDepth("Fiber1.jpg","Dry1.jpg","Wet1.jpg",HIDE,HIDE,SHOW,780,820,410,500,670,710,410,500) # Call the function with three files and three ints that tell the function whether to print images and debugging
filmDepth("Fiber2.jpg","Dry2.jpg","Wet2.jpg",HIDE,HIDE,HIDE,800,840,410,500,670,710,410,500) # Call the function with three files and three ints that tell the function whether to print images and debugging
filmDepth("Fiber3.jpg","Dry3.jpg","Wet3.jpg",HIDE,HIDE,HIDE,800,840,410,500,670,710,410,500) # Call the function with three files and three ints that tell the function whether to print images and debugging
filmDepth("Fiber4.jpg","Dry4.jpg","Wet4.jpg",HIDE,HIDE,HIDE,780,820,410,500,670,710,410,500) # Call the function with three files and three ints that tell the function whether to print images and debugging
filmDepth("Fiber5.jpg","Dry5.jpg","Wet5.jpg",HIDE,HIDE,HIDE,820,860,410,500,670,710,410,500) # Call the function with three files and three ints that tell the function whether to print images and debugging


# frame = cv2.imread("Wet1.jpg",1)
# cv2.line(frame, (LEFT_SIDE,startOfScan), (RIGHT_SIDE,startOfScan), (178, 34, 34), 1)
# cv2.line(frame, (LEFT_SIDE,startOfScan+100), (RIGHT_SIDE,startOfScan+100), (178, 34, 34), 1)
# cv2.imshow("Title",frame)

labels = ['Meas 1','Meas 2','Meas 3','Meas 4','Meas 5']
x = [0,1,2,3,4]
data = np.array(film)   # Convert the python list to a numpy array.
plt.plot(data, 'bo')    # Plot the data with red dots.
plt.ylabel("Film Depth in Microns", fontsize = 24) # Change the y axis lable.
plt.xlabel("Measurements", fontsize = 24)   # Change the x axis label.
plt.title("Repeatibility of Film Depths in Discrete Measurements", fontsize = 28)
plt.xticks(x, labels, rotation='vertical')
# plt.ylim([0,2])
plt.show()              # Show the plot. 'q' will exit from it.


header = "Film Depth at Middle"
np.savetxt('BottomFilmDepth5Meas.dat',data,header = header)
